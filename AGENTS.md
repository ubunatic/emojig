...
* **`spec/` is the primary developer surface — the YAML specs ARE the code.**
  ...
  **`spec/.gen/`** (tracked so plain `zig build` works, embedded by build.zig —
  never edit by hand). JSON Schemas for YAML editor support live in
  **`spec/.schema/`** (`theme.schema.json`/`art.schema.json` are regenerated by
  `make gen-colors` — extend `scripts/gen_colors/main.go` when adding theme
  fields). Behavior values (ranking weights, terminal launch flags, help texts,
  interaction steps, MRU size) belong in a spec file, not in Zig — see
  `spec/search.yaml`, `spec/host.yaml`, `spec/settings.yaml`, `spec/layout.yaml`.
  Spec invariants are linted by `scripts/test_tui/spec_lint_test.go` (`go test ./...`).
  **Non-Zig-app spec tiers** live in subdirectories: `spec/web/`
  (`jsdemo.yaml`, `crt-theme.yaml` — consumed only by `website/`) and
  `spec/reels/` (wayreel recorder scripts) are *not* loaded by `src/spec.zig`;
  don't look for app behavior there.
...
* **2D Grid Navigation, Scrolling & Focus**:
  ...
  * **Doc screens are pager-like** (`help`/`about`/`status`): `Up`/`Down`/`PageUp`/`PageDown`/`Home`/`End` scroll; `q`/`Esc`/`Enter`/`Space` close → search; **any other printable char jumps back to search and seeds the query** (so the screen never traps the user); unbound keys ring the dead-key bell. Status hint: `view.scrollable` when the content overflows the viewport, else `view.default` (`q/Esc:close`). There is a single `about` page (the quad-block art generated from `spec/art.yaml` into `about_lines` via `make gen-art`); the former `about2`-`4` multi-page sequence was removed.
...
> **Read [`docs/SearchEngine.md`](docs/SearchEngine.md) before editing search logic, `spec/boxart.yaml`, `data/emoji.json`, or `spec/synonyms.yaml`.** It documents the greedy word-order trap, `isBoxArt` codepoint range (U+2500–U+259F only — keyboard symbols are exempt), synonym-vs-direct-tag tradeoffs, and ranking test guidelines.

Implemented at query time in `src/root.zig` with **zero heap allocations**:
* **Spec-driven weights**: all ranking weights (char/word-start/consecutive bonuses, gap/late-start/length penalties, exact-word bonus, plural/stem fallback malus, box-art/braille class penalties) live in **`spec/search.yaml`** and load lazily via `search_mod.getSearchSpec()`. The JS simulator mirrors them through `webspec.js` (`make jsdemo`). Tune the YAML, not the code — and keep `zig build test` green (ranking tests pin relative order).
...
* **Box Art & `b:` Filter**: `spec/boxart.yaml` adds 68 box-drawing/block glyphs (U+2500–U+259F) with systematic names (`top left double border`, `bottom right border round`, `dark shade`, …). The `b:` prefix filters to them; in general searches a fixed score penalty (`box_art_penalty`) ranks them below genuine emoji matches. `isBoxArt` (Zig) / `IsBoxArt` (Go) classify by codepoint range; the Go port appends the entries in `internal/emoji.Load()`.
...
3. **Forced GUI Mode (`emojig --gui`)**:
   * Bypasses TTY checks and forces the launching of a new floating GUI window. The host terminal is chosen by precedence — `EMOJIG_TERMINAL`, then `$TERMINAL` (if on PATH), then the **`spec/host.yaml` `detection` list** (`foot` preferred, else `ptyxis`/`kitty`/`alacritty`/`wezterm`/`ghostty`/`konsole`/`gnome-terminal`/`xterm`). Per-terminal launch flags are **argv templates in `spec/host.yaml`** (`{placeholder}` entries drop when their value is empty; see `src/host.zig` `ArgValues`) — add a new terminal by adding a spec entry, no Zig changes needed. `foot` keeps cell-precise sizing; others adapt via altscreen. Fails if no active Wayland or X11 graphical session is detected, or if no supported terminal is found (`EMOJIG_TERMINAL` hint is printed).
