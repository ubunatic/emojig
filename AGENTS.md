...
* **Selection Highlight & Theming**:
  ...
  * `system` theme is mode-sensitive: in TUI mode it queries the current terminal background via OSC 11; in GUI mode the parent launcher checks GNOME `color-scheme` first (`prefer-dark` => dark), then GTK theme name (`*dark*` => dark), and falls back to dark if GNOME detection is unavailable.
  ...
  * **Null color contract for caps and separators**: a `null` value on any cap (`search_{left,right}_cap_{fg,bg}`) or sep (`search_theme_sep_{fg,bg}`, `theme_settings_sep_{fg,bg}`) color resolves to `cap_fallback_idx` — the app canvas color (`app_bg` index if set, else nearest xterm-256 to the effective theme's `terminal_bg2`, painted explicitly). This "punch-through" effect makes the canvas show through at that slot without exposing an opposite-themed host terminal background. Never use `search_bg` as the fallback for null sep/cap colors — that was the source of the near-black separator bug (cap_fallback ≈ 236 for dark theme looked black against search_bg=238).
