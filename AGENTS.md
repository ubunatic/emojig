...
* **Selection Highlight**:
  * Highlight the active emoji using the POSIX `\x1b[7m` (reverse video) ANSI block. 
  * Do not use custom theme colors. Reverse video guarantees the highlight matches the user's dark or light terminal background.
* **2D Grid Navigation**:
