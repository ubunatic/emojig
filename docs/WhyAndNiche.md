---
title: Why Emojig — Niche & Design Goals
weight: 10
---
...
* **Minimal Memory Budget**: Operates with a Resident Set Size (RSS) under **2.5 MB** and a statically compiled binary size under **650 KB**.
...
flowchart TD
    ...
    subgraph Compilation (Zig)
        C --> D["zig build"]
        E["src/main.zig"] --> D
        D --> F["dist/emojig (Static Binary <650KB)"]
    end
...
* **Tiny Binary Footprint**: Zig does not compile garbage collection or complex scheduler bookkeeping into the target binary, allowing the release executable to fit within 650 KB.
