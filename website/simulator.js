...
class EmojigSimulator {
  ...
  render() {
    ...
    // Remaining spacer logic: Total 41. Prompt takes 3. Theme takes 4. Query takes up to 34.
    const maxQueryCols = 34;
    const displayQuery =
    ...
    const placeholderHtml =
      displayQuery.length === 0
        ? `<span class="sim-search-placeholder">search…</span><span class="sim-cursor">█</span><span>${" ".repeat(Math.max(0, padLen - 7))}</span>`
        : `<span class="sim-search-query">${displayQuery}</span><span class="sim-cursor">█</span><span>${" ".repeat(padLen)}</span>`;
    html +=
    ...
    // 8. Status Bar Row
    const statusText = ` ${totalMatches}  ←↑↓→|↵|Esc  Help:?`;
    html += `<div class="sim-row sim-status">${padRight(statusText, 41)}</div>`;
  ...
  seedShell() {
    this.shellLines = [
      {
        kind: "out",
        text: "# emojig shell widget — type a command, then press Ctrl+E",
      },
      {
        kind: "out",
        text: "# try: type 'echo ', hit Ctrl+E, pick an emoji, then Enter",
      },
    ];
    ...
    switch (cmd) {
      ...
      case "help":
        this.shellLines.push({
          kind: "out",
          text: "commands: echo <text>, clear, help",
        });
        this.shellLines.push({
          kind: "out",
