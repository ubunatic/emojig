...
-->
# 061 — canary shot artifacts land untracked in `git status` with no ignore rule or commit policy

**Status**: Closed — resolved (moot as of 2026-08-11; `scripts/canary_font/` moved to `../fontwidth` per issue 62, so there is nothing here to gitignore)
**Priority**: P3 (Low) (hygiene; no user-facing impact, but it produces steady
