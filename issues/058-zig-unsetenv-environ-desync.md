...
-->
# 058 — Zig 0.16 stdlib bug: `unsetenv()` desyncs `std.Io.Threaded`'s own `environ` cache

**Status**: Open — workaround shipped for one of the two symptoms
**Priority**: P3 (Low) (found while building a manual research canary, not on
any emojig code path used at runtime — `-unset=` in `canary_font.zig`
(now moved to `../fontwidth/canaries/`, see
[issue 62](archive/62-move-font-width-experiments-to-fontwidth.md)) is a
...
[issue 62](archive/62-move-font-width-experiments-to-fontwidth.md)) was the only
...
- `../fontwidth/canaries/canary_font.zig` — where this was originally
  ...
  [issue 62](archive/62-move-font-width-experiments-to-fontwidth.md).
