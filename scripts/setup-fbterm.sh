...
# ── 1. Install fbterm ──────────────────────────────────────────────────────────
if command -v fbterm &>/dev/null
then
    ok "fbterm already installed ($(command -v fbterm))"
else
    info "Installing fbterm..."
    if command -v apt-get &>/dev/null
    then sudo apt-get install -y fbterm
    elif command -v pacman &>/dev/null
    then sudo pacman -S --noconfirm fbterm
    elif command -v dnf &>/dev/null
    then sudo dnf install -y fbterm
    else die "Cannot detect package manager. Install fbterm manually and re-run."
    fi
fi

# ── 2. /dev/fb0 group membership ───────────────────────────────────────────────
FB_GROUP=$(stat -c '%G' /dev/fb0 2>/dev/null || echo "video")
if groups | grep -qw "$FB_GROUP"
then ok "User already in group '$FB_GROUP'"
else info "Adding $USER to group '$FB_GROUP' (requires logout/login to take effect)..."
...
if fc-list | grep -qi "Noto"
then ok "Noto fonts available"
else warn "Noto fonts not found — fbterm will fall back to a system monospace."
     warn "Install with: sudo apt-get install fonts-noto"
fi

if test -f "$FBTERM_CFG"
then ok ".fbtermrc already exists at $FBTERM_CFG — not overwriting"
else info "Writing $FBTERM_CFG..."
     cat > "$FBTERM_CFG" <<EOF
